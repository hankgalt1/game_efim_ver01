...
//Функция для обновления данных загрузочного экрана.
    update(){
        if(this.axeHP ==0){
            this.scene.start(SCENE_KEYS.GAME_OVER, GameOver);
        }
        ...
        if (this.input.activePointer.isDown ==true){
            ...
            if(this.block[this.index].texture.key == "gold"){
                console.log("Золотой")
                this.osk.push(this.physics.add.sprite(this.scale.width/1.4,this.scale.height/2,"moneta").setGravityX(-this.setvel).setGravityY(-500*1.5).setDisplaySize(this.setOskolok,this.setOskolok))
                this.ScoreGold++
